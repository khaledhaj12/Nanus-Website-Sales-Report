...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Import historical orders endpoint (used by API Connections page)
  app.post('/api/import-historical', isAuthenticated, async (req, res) => {
    try {
      const { startDate, endDate, platform } = req.body;
      
      // Get API settings for the platform
      const apiSettings = await storage.getRestApiSettings(platform);
      if (!apiSettings || !apiSettings.storeUrl || !apiSettings.consumerKey || !apiSettings.consumerSecret) {
        return res.status(400).json({ 
          success: false, 
          message: "API credentials not configured for this connection" 
        });
      }

      const { storeUrl, consumerKey, consumerSecret } = apiSettings;
      ...
      while (hasMore) {
        ...
        for (const order of orders) {
          try {
            ...
            // Auto-create location based on order metadata
            const locationMeta = order.meta_data?.find(meta => 
              meta.key === 'location' || meta.key === '_location' || meta.key === 'store_location'
            )?.value || 'Unknown Location';

            let location = await storage.getLocationByName(locationMeta);
            if (!location) {
              location = await storage.createLocation({
                name: locationMeta,
                address: `${order.billing?.address_1 || ''} ${order.billing?.city || ''} ${order.billing?.state || ''}`.trim() || 'Address not provided',
                isActive: true
              });
            }
            ...
            const orderData = {
              ...
              refundTotal: parseFloat(order.refunds?.reduce((sum, refund) => sum + parseFloat(refund.total || '0'), 0) || '0').toFixed(2),
              orderDate: new Date(order.date_created || new Date()),
              locationMeta,
              rawData: JSON.stringify(order),
              ...
              billingAddress1: order.billing?.address_1 || '',
              shippingAddress1: order.shipping?.address_1 || ''
            };
